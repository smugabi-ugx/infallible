...
// expo-notifications loaded lazily — avoids module-init crash on some devices
...
const C = {
  ...
  muted:   '#27272a', danger:  '#f43f5e', success: '#10b981',
}
...
export default function OnboardingScreen() {
  ...
  const handleServerCheck = async () => {
    ...
    try {
      ...
      setError("Can't reach that server. Check the URL and make sure the server is running.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <View style={styles.root}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          {step === 0 && (
            <StepContainer icon="🌐" title="Connect to your server"
              subtitle="Enter the address of the machine running the Infallible server.">
              <Field label="Server URL" value={serverUrl} onChangeText={setServerUrl}
                placeholder="http://192.168.x.x:3000" autoCapitalize="none" keyboardType="url" />
          ...
          {step === 1 && (
            <StepContainer icon="🔑" title="Enter your device token"
              subtitle="Paste the token from your Infallible dashboard after registering this device.">
              <Field label="Device Token" value={token} onChangeText={setToken}
                placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
                autoCapitalize="none" autoCorrect={false} mono />
          ...
          {step === 2 && (
            <StepContainer icon="🛡️" title="Grant permissions"
          ...
          {step === 3 && (
            <StepContainer icon="✅" title="You're protected."
              subtitle="Infallible is running in the background. Your device is now monitored.">
              {[
                '📍 GPS reporting every 60 seconds',
                '🔔 Commands delivered via Socket.io',
                '📷 Camera ready for unauthorised access',
                '🛡️ Stealth mode off — enable from dashboard',
              ].map(item => (
...
function Field({ label, mono, ...props }) {
  return (
    <View style={styles.fieldWrap}>
      ...
      <TextInput style={[styles.input, mono && styles.inputMono]}
        placeholderTextColor={C.sub} selectionColor={C.primary} {...props} />
    </View>
  )
}

function Hint({ text }) { return <Text style={styles.hint}>{text}</Text> }
...
const styles = StyleSheet.create({
  ...
  errorBanner: {
    ...
    borderColor: 'rgba(244,63,94,0.25)', borderRadius: 12, padding: 14,
  },
