...
export async function handleCommand({ type, commandId, payload }) {
  ...
  try {
    switch (type) {
      ...
      case 'stealth_off':
        ...
        break
      case 'tracking_high':  await startTracking('high');    break
      case 'tracking_low':   await startTracking('low');     break
      case 'tracking_off':   stopTracking();                 break
      default:
        status = 'failed'
  ...
  try { await apiPost(`/commands/ack/${commandId}`, { deviceToken, status, errorMessage }) } catch {}
}
